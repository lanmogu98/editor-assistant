...
class TestRunRepository:
    ...
    def test_input_preserves_metadata(self, repo):
        ...
        input_id = repo.get_or_create_input(
            "news", "/second.md", "Second Title", content
        )
    ...
    def test_search_by_title(self, repo):
        ...
        id1 = repo.get_or_create_input(
            "paper", "/a.pdf", "Quantum Computing Paper", "C1"
        )
        id2 = repo.get_or_create_input(
            "paper", "/b.pdf", "Machine Learning Paper", "C2"
        )
...
class TestEdgeCases:
    ...
    def test_very_long_content(self, repo):
        ...
        input_id = repo.get_or_create_input(
            "paper", "/big.pdf", "Big Paper", long_content
        )
...
class TestResumableRuns:
    ...
    def test_get_resumable_runs_includes_input_info(self, repo):
        ...
        input_id = repo.get_or_create_input(
            "paper", "/path/to/paper.pdf", "My Paper", "Content"
        )
        run_id = repo.create_run(
            "outline", "gemini-3-flash", [input_id], thinking_level="high"
        )
...
class TestExportRuns:
    ...
    def populated_repo(self, repo):
        ...
        run2 = repo.create_run(
            "outline", "gemini-3-flash", [id1], thinking_level="high"
        )
        repo.update_run_status(run2, "success")
    ...
    def test_export_runs_csv_escapes_special_chars(self, repo, temp_dir):
        ...
        input_id = repo.get_or_create_input(
            "paper", "/p.pdf", 'Title with "quotes" and, commas', "Content"
        )
        run_id = repo.create_run("brief", "model", [input_id])
        repo.update_run_status(run_id, "success")
...
class TestProductionDatabaseIsolation:
    ...
    def test_repository_uses_isolated_database_during_tests(
        self, isolate_database_from_production
    ):
