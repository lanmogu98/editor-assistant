...
class TestErrorBoundaries:
    ...
    async def test_rate_limit_handling(self, mock_httpx_client):
        ...
        error_response.raise_for_status.side_effect = httpx.HTTPStatusError("Rate limit", request=None, response=error_response)
        ...
        mock_httpx_client.post.side_effect = [error_response.raise_for_status.side_effect, error_response.raise_for_status.side_effect, success_response]
    ...
    async def test_network_timeouts(self, mock_httpx_client):
        ...
        mock_httpx_client.post.side_effect = [
            ...
            MagicMock(status_code=200, json=lambda: {"choices": [{"message": {"content": "Recovered"}}]})
        ]
    ...
    async def test_partial_batch_failure(self):
        ...
        with patch.object(processor, 'process_mds', side_effect=mock_process):
            inputs = [
                MDArticle(type=InputType.PAPER, content="good1", title="1"),
                MDArticle(type=InputType.PAPER, content="bad1", title="2"),
                MDArticle(type=InputType.PAPER, content="good2", title="3"),
            ]
    ...
    async def test_semaphore_saturation(self):
        ...
        with patch("editor_assistant.llm_client.LLMClient.generate_response", side_effect=mock_slow_llm), \
            ...
            mock_task_instance.build_prompt.return_value = "prompt"
            ...
            inputs = [MDArticle(type=InputType.PAPER, content=f"content{i}", title=f"{i}") for i in range(10)]
            tasks = [processor.process_mds([inp], "brief") for inp in inputs]
