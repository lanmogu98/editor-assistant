...
async def test_batch_process_ui_flow(tmp_path, capsys):
    ...
    with patch("editor_assistant.cli.EditorAssistant") as MockAssistant:
        ...
        with patch("editor_assistant.cli.RICH_AVAILABLE", True):
            ...
            with (
                patch("editor_assistant.cli.Console") as MockConsole,
                ...
                assert (
                    call_kwargs.get("transient") is True
                ), "Progress should be transient"
                assert (
                    call_kwargs.get("console") == MockConsole.return_value
                ), "Progress should use our forced console"
