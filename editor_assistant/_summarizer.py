...
Uses the ContentChunker to split papers into manageable chunks,
...
class Summarizer:
    ...
    def process_content(self, content_path: str) -> Dict[str, Any]:
        ...
        for i, chunk in enumerate(chunks):
            self._save_content("chunk", f"chunk_{i+1}", chunk, paper_output_dir)
        ...
        for i, chunk in enumerate(chunks):
            ...
            try:
                ...
                self._save_content("prompt", f"chunk_analysis_{i+1}", prompt, paper_output_dir)
                response = self._make_api_request(prompt, f"chunk_analysis_{i+1}")
                # Save the full response
                self._save_content("response", f"chunk_analysis_{i+1}", response["text"], paper_output_dir)
        ...
        if total_chunks > 1:
            ...
            try:
                ...
                self._save_content("response", "synthesis", synthesis_response["text"], paper_output_dir)
        ...
        try:
            ...
            self._save_content("prompt", "translation", translation_prompt, paper_output_dir)
        except Exception as e:
        ...
        try:
            ...
            self._save_content("response", "translation", translation_response["text"], paper_output_dir)
        ...
        try:
            self._save_process_times_report(paper_name, process_times, paper_output_dir)
        except Exception as e:
            logging.error(f"Error saving process times report: {str(e)}")
        
        # Save token usage report
        try:
            self.llm_client.save_token_usage_report(paper_name, paper_output_dir)
        except Exception as e:
    ...
    def _save_content(self, type:str, content_name: str, content: str, paper_output_dir: Path) -> None:
        """
    ...
    def _save_process_times_report(self, paper_name: str, process_times: Dict[str, float], paper_output_dir: Path) -> None:
        """
