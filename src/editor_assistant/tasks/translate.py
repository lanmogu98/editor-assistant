...
class TranslateTask(Task):
    """Generate Chinese translation with optional bilingual output."""
    ...
    def _create_bilingual_content(self, source: str, translated: str) -> str:
        """Create bilingual markdown with alternating lines."""
        ...
        for i in range(len(source_lines)):
            ...
            if i < len(trans_lines):
                ...
                warning(
                    f"Line count mismatch at line {i}: "
                    f"source has {len(source_lines)}, "
                    f"translation has {len(trans_lines)}"
                )
                break
