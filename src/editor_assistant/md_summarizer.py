...
class MDSummarizer:
    ...
    def __init__ (self, model_name: str):
        ...
        context_window = self.llm_client.context_window
        max_output_tokens = self.llm_client.max_tokens
        ...
        content_token_budget = (context_window - max_output_tokens - 
                                prompt_overhead)
    ...
    def summarize_md (self, content_path: str, type: ArticleType) -> bool:
        """
        ...
        for i, chunk in enumerate(chunks):
            try:
                prompt = SUMMARIZER_PROMPTS[type]["chunk_analysis_prompt"].\
                format(
                    chunk_number=i+1,
        ...
        translation_prompt = TRANSLATION_PROMPT.format(
            content=synthesis_response if synthesis_response else "",
            title=title
        )
...
def main():
    ...
    supported_models = LLMClient.get_supported_models()
    from .config.set_llm import LLMModel

    
    # set up the parser
    parser = ArgumentParser(description="MD Summarizer")
    ...
    parser.add_argument("--model", 
                        type = LLMModel,
                        default = LLMModel.deepseek_r1_latest, 
    ...
    # parse the arguments and call the summarizer
    args = parser.parse_args()
    summarizer = MDSummarizer(args.model)
    for content_path in args.content_paths:
        summarizer.summarize_md(content_path, args.type)
