...
class RunRepository:
    ...
    def get_or_create_input(
        self, input_type: str, source_path: str, title: str, content: str
    ) -> int:
        """
    ...
    def add_token_usage(
        ...
        cursor.execute(
            ...
            INSERT INTO token_usage (
                run_id, input_tokens, output_tokens, cost_input,
                cost_output, process_time
            )
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (
    ...
    def get_recent_runs(self, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        cursor.execute(
            ...
            SELECT
                ...
                t.input_tokens,
                t.output_tokens,
                COALESCE(t.cost_input, 0)
                    + COALESCE(t.cost_output, 0) as total_cost
            FROM runs r
    ...
    def get_run_details(self, run_id: int) -> Optional[Dict[str, Any]]:
        ...
        # Get outputs
        cursor.execute("SELECT * FROM outputs WHERE run_id = ?", (run_id,))
        run["outputs"] = [dict(row) for row in cursor.fetchall()]

        # Get token usage
        cursor.execute("SELECT * FROM token_usage WHERE run_id = ?", (run_id,))
    ...
    def search_by_title(
        self, title_pattern: str, limit: int = 20
    ) -> List[Dict[str, Any]]:
        """
    ...
    def export_runs(
        ...
        for row in cursor.fetchall():
            ...
            cursor.execute(
                """
                SELECT
                    input_tokens, output_tokens, cost_input,
                    cost_output, process_time
                FROM token_usage
                WHERE run_id = ?
            """,
        ...
        with open(output_path, "w", newline="", encoding="utf-8") as f:
            ...
            for run in runs:
                ...
                row = {
                    ...
                    "total_cost": (usage.get("cost_input") or 0)
                    + (usage.get("cost_output") or 0),
                }
                writer.writerow(row)
