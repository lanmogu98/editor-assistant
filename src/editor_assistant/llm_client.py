...
class LLMClient:
    ...
    def __init__(self, model_name: str, thinking_level: str = None):
        ...
        # Apply thinking_level override if provided.
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        ...
        if self._async_client is None:
            # Note: Without a context manager, we might leak connections.
            # Ideally, users should use 'async with LLMClient()'.
            # For backward compatibility, create a new one,
    ...
    async def _wait_for_rate_limit(self) -> None:
        ...
        if time_since_last < self._min_interval:
            ...
            if RATE_LIMIT_WARNINGS_ENABLED:
                warning(
                    f"Rate limiting: waiting {wait_time:.2f}s (min interval)"
                )
    ...
    async def generate_response(
        ...
        if self._cache_enabled:
            ...
            if cached_response is not None:
                ...
                # Return 0 usage to avoid double counting.
                empty_usage = {
                    "total_input_tokens": 0,
                    "total_output_tokens": 0,
                    "cost": {
        ...
        # Request token usage data for streaming responses.
        if stream:
        ...
        for attempt in range(MAX_API_RETRIES):
            try:
                ...
                # Use repr(e) because some httpx exceptions have empty __str__.
                # Beginner note:
                # - We want error messages to be actionable for users.
                # - Low-level transport issues may format to "" via str(e).
                # - repr(e) usually includes the exception class and params.
    ...
    async def _stream_response(
        ...
        # Estimate tokens if the streaming API does not return usage.
        if input_tokens == 0:
    ...
    def save_token_usage_report(
        ...
        report["total_process_time"] = token_usage["process_times"][
            "total_time"
        ]
        ...
        with open(
            output_dir / f"token_usage_{project_name}.txt",
            "w",
            encoding="utf-8",
        ) as f:
            ...
            f.write("Summary:\n")
            f.write(
                f"  Total Input Tokens: {token_usage['total_input_tokens']}\n"
            )
            ...
            input_cost = token_usage["cost"]["input_cost"]
            output_cost = token_usage["cost"]["output_cost"]
            total_cost = token_usage["cost"]["total_cost"]
            f.write(
                f"  Input Cost: {self.pricing_currency}" f"{input_cost:.6f}\n"
            )
            f.write(
                f"  Output Cost: {self.pricing_currency}"
                f"{output_cost:.6f}\n"
            )
            f.write(
                f"  Total Cost: {self.pricing_currency}"
                f"{total_cost:.6f}\n\n"
            )
            ...
            for i, req in enumerate(token_usage["requests"]):
                ...
                f.write(
                    f"    Process Time: {req['process_time']:.2f} seconds\n"
                )
                f.write(
                    f"    Input Cost: {self.pricing_currency}"
                    f"{req['input_cost']:.6f}\n"
                )
                f.write(
                    f"    Output Cost: {self.pricing_currency}"
                    f"{req['output_cost']:.6f}\n"
                )
                f.write(
                    f"    Total Cost: {self.pricing_currency}"
                    f"{req['total_cost']:.6f}\n\n"
        ...
        total_time = token_usage["process_times"]["total_time"]
        progress(
            f"Token usage: {total_tokens} tokens "
            f"({self.pricing_currency}{total_cost:.4f}) "
        ...
        logger.debug(
            "Detailed token usage report saved to: "
            f"{output_dir / 'token_usage.txt'}"
        )
